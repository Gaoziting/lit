...
switch (isset($_POST['type'])?$_POST['type']:'') {
	case 'uname':
		...
		if ($rst->rowCount()>0) {
			...
			$arr = array('status'=>200,'type'=>'uname','uname'=>$uname,'detail'=>'修改成功');
	...
	case 'school':
		...
		if ($rst->rowCount()>0) {
			...
			$row = $rst->fetch();
			$school = $row[0];
			$arr = array('status'=>200,'type'=>'school','school'=>$school,'detail'=>'修改成功');
	...
	case 'intro':
		...
		if ($rst->rowCount()>0) {
			...
			$row = $rst->fetch();
			$intro = $row[0];
			$arr = array('status'=>200,'type'=>'intro','intro'=>$intro,'detail'=>'修改成功');
